...
if __name__ == "__main__":
    ...
    rust_code_dir_path = input(
        "What is rust_code_dir_path? (e.g. src/)\n > "
    ).strip()

    # Accept a single name OR comma-separated list of names
    function_names_raw_input = input(
        "What function name(s) to find? "
        "(single name, or comma-separated list for multiple)\n > "
    ).strip()

    # Parse: split on comma, strip whitespace, drop empty tokens
    function_names_list: list[str] = [
        name.strip()
        for name in function_names_raw_input.split(",")
        if name.strip()
    ]

    if not function_names_list:
        print("ERROR: No function names provided. Exiting.")
        sys.exit(1)

    function_depth_input = input(
        "(optional) What is function_depth? (default 0)\n > "
    ).strip()

    file_depth_input = input(
        "(optional) What is file_depth? (default 2)\n > "
    ).strip()

    only_search_this_file_path_input = input(
        "(optional) What is only_search_this_file_path?\n > "
    ).strip()

    output_dir_input = input(
        "(optional) What is output_dir?\n > "
    ).strip()

    # Parse optional inputs, falling back to defaults when empty
    function_depth: int = int(function_depth_input) if function_depth_input else 0
    file_depth: int    = int(file_depth_input)      if file_depth_input      else 2

    only_search_this_file_path: str | None = (
        only_search_this_file_path_input if only_search_this_file_path_input else None
    )
    output_dir: str | None = (
        output_dir_input if output_dir_input else None
    )

    # ------------------------------------------------------------------ #
    #  Loop over every requested function name and call function_finder   #
    # ------------------------------------------------------------------ #

    # Accumulate every non-error-log file produced across all function calls
    all_extracted_rs_file_paths_list: list[str] = []
    # Accumulate error log paths separately (one per function_finder call)
    all_error_log_file_paths_list: list[str] = []

    print(f"\nProcessing {len(function_names_list)} function name(s)...\n")

    for single_function_name_string in function_names_list:
        print(f"  -> Searching for: {single_function_name_string!r}")
        try:
            produced_file_paths_list = function_finder(
                rust_code_dir_path=rust_code_dir_path,
                function_name_to_find=single_function_name_string,
            ...
            # Separate extracted .rs files from error logs by checking
            # for the known "error_log_" substring in the filename.
            # FIX 1: use a single clear condition (not the redundant double-check)
            # FIX 2: report the actual counts rather than doing - 1 arithmetic
            extracted_rs_files = [
                f for f in produced_file_paths_list
                if "error_log_" not in os.path.basename(f)
            ]
            error_log_files = [
                f for f in produced_file_paths_list
                if "error_log_" in os.path.basename(f)
            ]

            all_extracted_rs_file_paths_list.extend(extracted_rs_files)
            all_error_log_file_paths_list.extend(error_log_files)

            print(
                f"     OK  {len(extracted_rs_files)} extracted .rs file(s) | "
                f"error log: {error_log_files[0] if error_log_files else 'none'}"
            )

        except Exception as per_function_exception_object:
            print(f"     ERR {single_function_name_string!r}: {per_function_exception_object!r}")

    # ------------------------------------------------------------------ #
    #  Summary                                                            #
    # ------------------------------------------------------------------ #
    print(f"\nExtracted files ({len(all_extracted_rs_file_paths_list)} total):")
    for single_extracted_path in all_extracted_rs_file_paths_list:
        print(f"  {single_extracted_path}")

    # ------------------------------------------------------------------ #
    #  Optional flatten                                                   #
    # ------------------------------------------------------------------ #
    flatten_or_not = input(
        "\nDo you want a flat file?    (y)es / (n)o\n > "
    ).strip()

    if flatten_or_not.lower() in {"y", "yes", "true", "ok", "flat"}:
        flat_output_dir = output_dir if output_dir else "./function_finder_files"
        flat_path_string = flatten_finder(flat_output_dir)
        print(f"\nFlat file written: {flat_path_string}")

    print(f"\nDone. Processed {len(function_names_list)} function name(s).\n")
